...
class PlaceTestCase(APITestCase):
    ...
    def test_place_pagination(self):
        ...
        response = self.client.get(
            reverse("place-list") + f"?page_size={page_size}&ordering=name"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        response = self.client.get(
            reverse("place-list")
            + f"?page_size={page_size}&ordering=name&page=2"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        for page in range(1, page_count + 1):
            response = self.client.get(
                reverse("place-list") + f"?page_size={page_size}&page={page}"
            )
            self.assertEqual(response.status_code, status.HTTP_200_OK)
            if page != page_count:
    ...
    def test_place_filter_name(self):
        ...
        response = self.client.get(
            reverse("place-list") + f"?name={self.place.name}"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data["count"],
            Place.objects.filter(name=self.place.name).count(),
        )
        self.assertEqual(response.data["results"][0]["name"], self.place.name)

        response = self.client.get(
            reverse("place-list") + f"?name={place1.name}"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        response = self.client.get(
            reverse("place-list") + f"?name={self.place.name[:-3]}"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_place_filter_address(self):
        ...
        response = self.client.get(
            reverse("place-list") + f"?address={self.place.address}"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["count"], 1)
        self.assertEqual(
            response.data["results"][0]["address"], self.place.address
        )

        response = self.client.get(
            reverse("place-list") + f"?address={place1.address}"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["count"], 1)
        self.assertEqual(
            response.data["results"][0]["address"], place1.address
        )

        response = self.client.get(
            reverse("place-list") + f"?address={place1.address[:-3]}"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["count"], 0)
    ...
    def test_place_filter_search_name_exact_match(self):
        ...
        response = self.client.get(
            reverse("place-list") + f"?search={self.place.name}"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data["count"],
    ...
    def test_place_filter_search_name_contains(self):
        ...
        response = self.client.get(
            reverse("place-list") + f"?search={self.place.name[:-3]}"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data["count"],
            Place.objects.filter(name__icontains=self.place.name[:-3]).count(),
        )
        self.assertEqual(response.data["results"][0]["name"], self.place.name)

        self.assertEqual(
            Place.objects.filter(name__icontains="Place").count(),
            self.place_count,
        )
        ...
        self.assertEqual(Place.objects.count(), self.place_count + 4)
        response = self.client.get(
            reverse("place-list") + "?search=ObjectToTest"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_place_filter_search_address_exact_match(self):
        ...
        response = self.client.get(
            reverse("place-list") + f"?search={self.place.address}"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data["count"],
            Place.objects.filter(address=self.place.address).count(),
        )
        self.assertEqual(
            response.data["results"][0]["address"], self.place.address
        )
    ...
    def test_place_filter_search_address_contains(self):
        ...
        response = self.client.get(
            reverse("place-list") + f"?search={test_place.address}"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["count"], 1)
        self.assertEqual(
            response.data["results"][0]["address"], test_place.address
        )

        self.assertEqual(
            Place.objects.filter(address__icontains="Test Address").count(), 3
        )
        response = self.client.get(
            reverse("place-list") + "?search=Test Address"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data["count"],
            Place.objects.filter(address__icontains="Test Address").count(),
        )
    ...
    def test_place_filter_city(self):
        ...
        self.assertEqual(
            response.data["count"],
            Place.objects.filter(city__id=city.id).count(),
        )
        self.assertEqual(
    ...
    def test_place_filter_category_exact(self):
        ...
        response = self.client.get(
            reverse("place-list") + f"?category={category.id}"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data["count"],
            Place.objects.filter(category__id=category.id).count(),
        )
        self.assertEqual(
    ...
    def test_place_filter_category_contains(self):
        ...
        response = self.client.get(
            reverse("place-list")
            + f"?category={category.id},{new_category.id}"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data["count"],
            Place.objects.filter(category__id=category.id).count() + 3,
        )
        self.assertEqual(
            response.data["count"],
            Place.objects.filter(
                category__in=[category.id, new_category.id]
            ).count(),
        )
    ...
    def test_place_filter_rating_exact(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data["count"], Place.objects.filter(rating=1.9).count()
        )
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data["count"], Place.objects.filter(rating=2.9).count()
        )
        self.assertEqual(response.data["results"][0]["rating"], 2.9)

    def test_place_filter_rating_range(self):
        self.assertEqual(
            Place.objects.filter(rating__gte=3, rating__lte=4).count(), 0
        )
        ...
        self.assertEqual(
            Place.objects.filter(rating__gte=3, rating__lte=4).count(), 5
        )

        response = self.client.get(
            reverse("place-list") + "?rating_min=3&rating_max=4"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data["count"],
            Place.objects.filter(rating__gte=3, rating__lte=4).count(),
        )

        response = self.client.get(
            reverse("place-list") + "?rating_min=4&rating_max=5"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data["count"],
            Place.objects.filter(rating__gte=4, rating__lte=5).count(),
        )
    ...
    def test_place_filter_price_level_exact(self):
        ...
        response = self.client.get(
            reverse("place-list") + "?price_level=10$ - 25$"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["count"], 1)
        self.assertEqual(
            response.data["results"][0]["price_level"], "10$ - 25$"
        )

        response = self.client.get(
            reverse("place-list") + "?price_level=15$ - 35$"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_place_filter_price_level_contains(self):
        ...
        response = self.client.get(
            reverse("place-list") + "?price_level=10$ - 25$,15$ - 35$"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["count"], 3)
    ...
    def test_place_filter_opening_status(self):
        ...
        response = self.client.get(
            reverse("place-list") + "?opening_status=OPEN"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["count"], 3)
        ...
        response = self.client.get(
            reverse("place-list") + "?opening_status=CLOSED"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["count"], 2)
        self.assertEqual(
            response.data["results"][0]["opening_status"], "CLOSED"
        )
    ...
    def test_place_filter_radius(self):
        ...
        # In radius of 10km
        response = self.client.get(
            reverse("place-list") + f"?lat={lat}&lon={lon}&radius=10"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["count"], 3)
        ...
        # In radius of 50km
        response = self.client.get(
            reverse("place-list") + f"?lat={lat}&lon={lon}&radius=50"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        # In radius of 100km
        response = self.client.get(
            reverse("place-list") + f"?lat={lat}&lon={lon}&radius=100"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_place_display_distance_with_users_coordinates(self):
        response = self.client.get(
            reverse("place-list") + "?lat=48.8566&lon=2.3522"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        response = self.client.get(
            reverse("place-list")
            + "?name=Distance Test&lat=48.8566&lon=2.3522"
        )
    ...
    def test_place_city_invalid_params(self):
        response = self.client.get(
            reverse("place-list") + "?city=invalid_param"
        )
    ...
    def test_place_category_invalid_params(self):
        response = self.client.get(
            reverse("place-list") + "?category=invalid_param"
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        ...
        response = self.client.get(
            reverse("place-list")
            + "?category=invalid_param,another_invalid_param"
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIsInstance(response.data["category"][0], ErrorDetail)
    ...
    def test_place_rating_invalid_params(self):
        response = self.client.get(
            reverse("place-list") + "?rating=invalid_param"
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_place_rating_min_invalid_params(self):
        response = self.client.get(
            reverse("place-list") + "?rating_min=invalid_param"
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_place_rating_max_invalid_params(self):
        response = self.client.get(
            reverse("place-list") + "?rating_max=invalid_param"
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_place_opening_status_invalid_params(self):
        response = self.client.get(
            reverse("place-list") + "?opening_status=invalid_param"
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_place_radius_invalid_params(self):
        response = self.client.get(
            reverse("place-list") + "?radius=invalid_param&lat=2&lon=3"
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_place_radius_can_not_be_negative(self):
        response = self.client.get(
            reverse("place-list") + "?radius=-100&lat=-90&lon=180"
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIsInstance(response.data["radius"][0], ErrorDetail)
    ...
    def test_place_lat_invalid_params(self):
        response = self.client.get(
            reverse("place-list") + "?lat=invalid_param"
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_place_lon_invalid_params(self):
        response = self.client.get(
            reverse("place-list") + "?lon=invalid_param"
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
