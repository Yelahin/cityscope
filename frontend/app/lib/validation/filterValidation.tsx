  ...
  if (
    ...
    category
      ...
      .every(
        (cat) =>
          isNumeric(cat) &&
          categories.some((option) => Number(cat) === option.id),
      )
  )
    return true;
  return false;
...
export function cityCheck(
  ...
  if (
    city &&
    isNumeric(city) &&
    cities.some((option) => Number(city) === option.id)
  )
    return true;
  return false;
}
...
export function priceLevelCheck(
  priceLevel: string | null,
  priceLevels: FilterOption[],
): boolean {
  if (
    priceLevel &&
    priceLevel
      .split(",")
      .every((price) => priceLevels.some((option) => option.name === price))
  )
    return true;
  return false;
}
...
export function openingStatusCheck(openingStatus: string | null): boolean {
  if (openingStatus && (openingStatus === "OPEN" || openingStatus === "CLOSED"))
    return true;
  return false;
}
