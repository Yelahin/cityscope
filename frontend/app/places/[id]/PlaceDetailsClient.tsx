...
export default function PlaceDetailsClient () {
    ...
    if (!place) {
        return <div className="flex h-main-content items-center justify-center"><Spinner /></div>;
    }

    const openingStatus = place.opening_status === "OPEN"
        ? "Open"
        : place.opening_status === "CLOSED" ? "Closed" : "Unknown";
    ...
    return (
        <div className="mx-auto flex max-w-5xl flex-col gap-6 px-5 py-8">
            ...
            <dl className="grid gap-4 rounded-xl border border-border-primary bg-neutral-900 p-5 sm:grid-cols-2">
                <div><dt className="text-sm text-gray-400">Address</dt><dd>{place.address ?? "Not available"}</dd></div>
                <div><dt className="text-sm text-gray-400">Category</dt><dd>{place.category.name}</dd></div>
                <div>
                ...
                <div><dt className="text-sm text-gray-400">Price level</dt><dd>{place.price_level ?? "Not available"}</dd></div>
                <div>
                    <dt className="text-sm text-gray-400">Opening status</dt>
                    <dd className={place.opening_status === "OPEN" ? "text-green-500" : place.opening_status === "CLOSED" ? "text-red-500" : ""}>
                        {openingStatus}
                    </dd>
                </div>
                <div><dt className="text-sm text-gray-400">City</dt><dd>{place.city.name}</dd></div>
            </dl>
