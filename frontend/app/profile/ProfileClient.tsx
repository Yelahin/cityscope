...
export default function ProfileClient() {
  ...
  async function deleteSearch(search: SavedSearch) {
    try {
      await fetchApi(`searches/${search.id}/`, undefined, undefined, {
        method: "DELETE",
      });
        ...
        {favorites.length === 0 ? (
          ...
          <ul className="grid gap-4 sm:grid-cols-2">
            {favorites.map((place) => (
              <li
                key={place.id}
                className="rounded-xl border border-border-primary bg-neutral-900 p-4"
              >
                <Link
        ...
        {searches.length === 0 ? (
          ...
          <ul className="flex flex-col gap-3">
            {searches.map((search) => (
              <li
                key={search.id}
                className="flex items-center justify-between gap-4 rounded-xl border border-border-primary bg-neutral-900 p-4"
              >
                <Link
                  href={`/?${getSearchQuery(search.params)}`}
