...
export default function ListFilter({
  ...
  setSelectedFilters: Dispatch<
    SetStateAction<Record<string, number | string | number[] | string[]>>
  >;
  selectById?: boolean;
  limit: number;
  ...
  const objArray = Array.isArray(selectedFilters[key])
    ? selectedFilters[key]
    : [];
        ...
        {filtered.map((object) => {
          ...
          return (
            <ListItem
              ...
              onClick={() => {
                setSelectedFilters((prev) => {
                  const currentArray: (string | number)[] = Array.isArray(
                    prev[key],
                  )
                    ? prev[key]
                    : [];
                  ...
                  if (currentArray.includes(value)) {
                    ...
                    updated = limit
                      ? limit === 1
                        ? [value]
                        : [...currentArray, value].length <= limit
                          ? [...currentArray, value]
                          : currentArray
                      : [...currentArray, value];
                  }
                ...
                if (setRequiredToSelect)
                  setRequiredToSelect((prev) =>
                    prev.filter((fil) => fil !== key),
                  );
              }}
            >
              {object.name}
