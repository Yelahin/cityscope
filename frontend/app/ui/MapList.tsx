...
export default function MapList({
        ...
        <ul className="mt-24 w-full h-[calc(var(--height-main-content)-calc(var(--height-header)*1.75))] overflow-auto overscroll-contain">
          ...
          >
            {displayedPlaces.map((place: Place) => {
              const priceLevel = (
                <p className="text-gray-400 text-sm">{place?.price_level}</p>
              );
              const openingStatus = (
                <p
  ...
  return (
    <>
      ...
      >
        <span
          className={`flex h-1 rounded-3xl transition-all
                    ${isOpen ? "w-8 bg-primary rotate-45 translate-y-2.5" : "w-7 bg-gray-500"}`}
        ></span>
        <span
          className={`flex h-1 rounded-3xl transition-all
      ...
      <div
        className={`-z-1 top-header left-0 w-full max-w-75 h-main-content bg-dark-primary
                ${isOpen ? "fixed" : "hidden"}`}
