...
describe('MapController', () => {
   it('should display markers for each place', () => {
    ...
    render(<MapContainer><MapController places={places} markersRef={markersRef} /></MapContainer>);
    ...
    expect(screen.getAllByText(`City: ${places[0].city['name']}`)).toHaveLength(5);
    expect(screen.getAllByText(`Category: ${places[0].category['name']}`)).toHaveLength(5);
   });
   ...
   it('should trigger flyToBounds when places change', async () => {
    ...
    expect(flyToBounds).toHaveBeenCalledWith(L.latLngBounds([[places[0].latitude, places[0].longitude]]), {maxZoom: 14, duration: 2, padding: [100, 100]});
   });
