...
describe('MapList', () => {
    it('should contain burger button', () => {
        ...
        render(<MapList places={[] as Place[]} onPlaceClick={() => null} isLoading={false} error={null} />);
    ...
    it('should return empty search state for empty search', () => {
        ...
        const { rerender } = render(<MapList places={[] as Place[]} onPlaceClick={() => null} isLoading={false} error={null} />);

        const emptyResult = screen.getByTestId('empty-results');
        expect(emptyResult).toBeInTheDocument();

        rerender(<MapList places={places} onPlaceClick={() => null} isLoading={false} error={null} />);
    ...
    it('should return loading state if places isLoading set to true', () => {
        ...
        const { rerender } = render(<MapList places={[] as Place[]} onPlaceClick={() => null} isLoading={true} error={null} />);
    ...
    it('should return error state if error was provided in props', () => {
        ...
        const { rerender } = render(<MapList places={[] as Place[]} onPlaceClick={() => null} isLoading={false} error={"Test-Error"} />);
    ...
    it('should return list of places when found results', () => {
        ...
        render(<MapList places={places} onPlaceClick={() => null} isLoading={false} error={null} />);
        ...
        expect(screen.getByText(places[0].address)).toBeInTheDocument();
        expect(screen.getByText(`Distance: ${String(places[0].distance)}km`)).toBeInTheDocument();
    });
    ...
    it('should contain link to detail page', () => {
        ...
        render(<MapList places={[places[0]]} onPlaceClick={() => null} isLoading={false} error={null} />);
    ...
    it('should trigger onPlaceClick function when clicked on map list item', async () => {
        ...
        let count = 0;
        render(<MapList places={places} onPlaceClick={() => count++} isLoading={false} error={null} />);
